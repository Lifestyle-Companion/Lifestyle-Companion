HEALTHY EATING COMPANION — FOUNDER TRIAL ALPHA 0.6.28

SEARCH INTELLIGENCE + QUICK FOOD LOG FOUNDATION

Deployment
1. Back up the existing HEC site/data before a major founder-test change.
2. Replace the complete deployed folder with Alpha 0.6.28 at the same GitHub Pages address.
3. Do not mix runtime files from older builds.
4. Wait for the GitHub Pages green tick, then reopen HEC and confirm Alpha 0.6.28 is visible.
5. Existing browser storage keys remain unchanged so founder-test data can migrate in place.

Core Alpha 0.6.28 rules
...
- Source/origin uses food-appropriate wording (for example Homemade vs Commercial/Bought bread) and is asked early only when it materially narrows the food search.
...
- Quick Food Log starts with the entry method — Keyboard, Voice, Barcode or Nutrition Panel — then confirms date/meal before anything is saved. Keyboard/Voice use the same canonical Food Search engine.
