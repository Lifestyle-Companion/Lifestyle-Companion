...
const APP = window.HEC_APP || {name:"Healthy Eating Companion",version:"0.6.32",storageKey:"healthyEatingCompanionAlpha06",functionalStorageKey:"healthyEatingCompanionAlpha06Functional"};
...
function cleanMeasureText(value){return String(value||'').replace(/(\d+)\.0+(?=\s*(?:g|ml)\b)/gi,'$1').replace(/(\d+\.\d*?[1-9])0+(?=\s*(?:g|ml)\b)/gi,'$1').replace(/\s+/g,' ').trim();}
...
function updateEntryPreview(){
  ...
  const scoreInputs=['protein','carbs','fat','satFat','fibre','sugar','sodium'].filter(k=>values?.[k]!==null&&values?.[k]!==undefined&&values?.[k]!==''&&Number.isFinite(Number(values[k]))).length,canScore=scoreInputs>=4;
  by("entry-nutrition-preview").innerHTML=`<div class="food-detail-title"><div><h3>${esc(food.name)}</h3><p>${esc(food.brand||"")} · ${esc(cleanMeasureText(food.serving||""))}</p></div>${canScore?`<span class="health-score score-${Math.min(10,whole(food.score))}">${whole(food.score)}/10</span>`:'<span class="health-score">Not scored</span>'}</div>${nutritionCards(values)}<p class="fine"><strong>${canScore?'Why This Score':'Nutrition Score'}:</strong> ${canScore?esc(scoreExplanation(food.score)):'Not enough verified nutrient information is available to score this food reliably.'}</p>`;
  if(by("entry-selection-summary"))by("entry-selection-summary").innerHTML=`<strong>${editorState?.libraryOnly?'Food To Save':'You Are Adding'}: ${formatNaturalAmount(amount)} ${esc(friendlyUnitLabel(food,unit,amount))}</strong><span>${energyText(values.calories)} · ${editorState?.libraryOnly?'Save now and use it from your Food Library later.':'Recorded in the Diary immediately'}</span>`;
}
...
function renderFoodLiveMatches(query){
  ...
  if(typeof alpha0618R2GuideLabels==="function" && typeof alpha0618R2Text==="function"){
    ...
    if(/\bburger king\b/.test(qn)){
      box.innerHTML='<div class="live-match-heading"><strong>Top Matches</strong><small>Australian location rules applied · Alpha 0.6.32</small></div><button type="button" class="live-match-row alpha0618-r2-guide-row" data-alpha0618-r2-guide="Hungry Jacks"><span><strong>Hungry Jack’s</strong><small>Australian equivalent — guided menu</small></span><b>›</b></button>';
    ...
    if(chain){
      ...
      box.innerHTML=`<div class="live-match-heading"><strong>Top Matches</strong><small>Australian menu first · Alpha 0.6.32</small></div>${items.map(x=>`<button type="button" class="live-match-row" data-alpha0618-chain-item="${esc(chain.label)}|${esc(x)}"><span><strong>${esc(x)}</strong><small>${esc(chain.label)}</small></span><b>＋</b></button>`).join("")}`;
      box.classList.remove("hidden");return;
    }
    ...
    box.innerHTML=`<div class="live-match-heading"><strong>Top Matches</strong><small>Guided Food Entry · Alpha 0.6.32</small></div>${guideHtml}`;
    box.classList.remove("hidden");return;
  }
...
document.addEventListener("click",event=>{
  const one=event.target.closest("[data-recent-entry-add]");if(one){const found=findEntry(one.dataset.recentEntryAdd);if(!found)return;const targetDate=ext.ui.recentPlanMode?(ext.ui.plannerDate||isoToday()):(ext.ui.diaryDate||isoToday()),filter=ext.ui.recentMealFilter||"",targetMeal=(filter&&filter!=="All")?filter:(ext.ui.pendingMeal||found.entry.meal);const copy=copyRecentEntry(found.entry,targetDate,targetMeal);saveExt();showActionToast(`${copy.name} added to ${targetMeal}.`,null,2000);return;}
  const meal=event.target.closest("[data-recent-meal-add]");if(meal){const [sourceDate,sourceMeal]=meal.dataset.recentMealAdd.split("|"),items=entriesForDate(sourceDate).filter(e=>e.meal===sourceMeal&&e.status!=="skipped"),targetDate=ext.ui.recentPlanMode?(ext.ui.plannerDate||isoToday()):(ext.ui.diaryDate||isoToday()),filter=ext.ui.recentMealFilter||"",targetMeal=(filter&&filter!=="All")?sourceMeal:(ext.ui.pendingMeal||sourceMeal);items.forEach(e=>copyRecentEntry(e,targetDate,targetMeal));saveExt();showActionToast(`${items.length} ${items.length===1?"item":"items"} added to ${targetMeal}.`,null,2000);return;}
});
...
function makeOpenFoodFactsFood(product){
  ...
  const coherentServingValue=(key)=>{const direct=servingValue(key),per100=per100Value(key),scaled=per100===null?null:per100*factor;if(direct===null)return scaled??0;if(scaled===null||scaled===0)return direct;const delta=Math.abs(direct-scaled)/Math.max(Math.abs(scaled),.001);if(delta<=.25)return direct;basisWarnings.push(key);return scaled;};
  let calories=coherentServingValue("energy-kcal");
  if(!calories){const directKj=servingValue("energy-kj"),per100Kj=per100Value("energy-kj"),scaledKj=per100Kj===null?null:per100Kj*factor;if(directKj!==null&&scaledKj!==null&&Math.abs(directKj-scaledKj)/Math.max(Math.abs(scaledKj),1)>.25){basisWarnings.push("energy-kj");calories=scaledKj/4.184;}else if(directKj!==null)calories=directKj/4.184;else if(scaledKj!==null)calories=scaledKj/4.184;}
  const nutrients={calories:n(calories),protein:coherentServingValue("proteins"),carbs:coherentServingValue("carbohydrates"),fat:coherentServingValue("fat"),satFat:coherentServingValue("saturated-fat"),fibre:coherentServingValue("fiber"),sugar:coherentServingValue("sugars"),sodium:coherentServingValue("sodium")*1000};
...
function alpha0631UsableOnlineFood(food){const cal=Number(food?.nutrients?.calories);if(!food?.id||!food?.nutrients||!Number.isFinite(cal))return false;if(cal>0)return true;const n=s23Norm(`${food?.name||''} ${food?.brand||''}`);return cal===0&&/\bwater\b|\bzero sugar\b|\bsugar free\b|\bdiet soft drink\b/.test(n);}
function upsertOnlineFoods(foods){foods.forEach(food=>{if(!alpha0631UsableOnlineFood(food))return;const i=ext.onlineFoods.findIndex(x=>x.id===food.id);if(i>=0)ext.onlineFoods[i]=food;else ext.onlineFoods.push(food);});alpha0630PruneOnlineFoods();alpha0630InvalidateFoodSearchCaches();}
...
function alpha0630HandleFoodSearchInput(){
  const input=by("food-search");if(!input)return;const value=input.value;ext.ui.foodSearch=value;const token=++alpha0630FoodSearchUiToken;
  ...
  alpha0630FoodSearchTimer=setTimeout(()=>{if(token!==alpha0630FoodSearchUiToken||!q('#food-library.active'))return;renderLibrary();keepLiveFoodResultsVisible();scheduleAllResourcesOnlineSearch();},70);
}
function alpha0631ExternalSearchQuery(query){
  const entity=REG29?.primary?.(query,['brand','retailer']);
...
function alpha0631RenderOnlineProgress(query,loaded,checkingMore=false){
  if((by("food-search")?.value.trim()||"")!==query||!q('#food-library.active'))return;
...
async function runOnlineFoodSearch({automatic=false}={}){
  const query=by("food-search")?.value.trim()||"",externalQuery=alpha0631ExternalSearchQuery(query),token=++onlineSearchToken;if(!query||query.length<3){if(!automatic)showActionToast("Enter at least three letters before searching online.",null,5000);return [];}
  ...
  const hasCommercialEntity=!!REG29?.primary?.(query,['brand','retailer']);const all=[];
  try{
    ...
    try{
      const off=await offPromise;if(token!==onlineSearchToken||signal.aborted)return[];if(off?.length){upsertOnlineFoods(off);all.push(...off);}alpha0631RenderOnlineProgress(query,all.length,!!usdaPromise);
    }catch(error){if(error?.name==='AbortError'||token!==onlineSearchToken)return[];}
    if(usdaPromise){try{const usda=await usdaPromise;if(token!==onlineSearchToken||signal.aborted)return all;if(usda?.length){upsertOnlineFoods(usda);all.push(...usda);}alpha0631RenderOnlineProgress(query,all.length,false);}catch(error){if(error?.name==='AbortError'||token!==onlineSearchToken)return all;alpha0631RenderOnlineProgress(query,all.length,false);}}
...
function parseNutritionPanel(text){
  ...
  const servingMatch=clean.match(/serv(?:ing|e|lng)?\s*(?:size|slze)[^\d]{0,28}(\d+(?:[.,]\d+)?)\s*(g|ml)\b/i)||clean.match(/serve\s*(?:size|slze)[^\d]{0,28}(\d+(?:[.,]\d+)?)\s*(g|ml)\b/i)||clean.match(/per\s*serve[^\d]{0,20}(\d+(?:[.,]\d+)?)\s*(g|ml)\b/i);
...
function alpha0631ScaleOcrReview(){
  if(!ocrParsedPanel)return;const basis=by("ocr-basis")?.value||ocrParsedPanel.basis||'serving',amount=n(by("ocr-serving-amount")?.value),unit=by("ocr-serving-unit")?.value||'g';
  const base=basis==='100'?ocrParsedPanel.per100:ocrParsedPanel.perServing;
  let factor=1;if(basis==='100')factor=(unit==='serving'?amount:(amount||100)/100);else if(ocrParsedPanel.servingAmount&&unit!=='serving')factor=(amount||ocrParsedPanel.servingAmount)/ocrParsedPanel.servingAmount;else factor=amount||1;
  Object.entries(ALPHA0631_OCR_FIELDS).forEach(([k,id])=>{if(!by(id))return;const v=base?.[k];by(id).value=(v===undefined||v===null||v==='')?'':round1(n(v)*factor);});
  const status=by('ocr-scale-status');if(status){const baseText=basis==='100'?'per 100 g / 100 mL':(ocrParsedPanel.servingAmount?`per package serving (${formatNumber(ocrParsedPanel.servingAmount,true)} ${ocrParsedPanel.servingUnit})`:'per serving');status.textContent=`Nutrition recalculates from the recognised ${baseText} values as you change the amount.`;}
}
function fillOcrReview(parsed,basis=parsed?.basis||"serving"){
  if(!parsed)return;ocrParsedPanel=parsed;if(by("ocr-basis"))by("ocr-basis").value=basis;if(basis==="serving"){by("ocr-serving-amount").value=parsed.servingAmount||1;by("ocr-serving-unit").value=parsed.servingAmount?parsed.servingUnit:"serving";}else{by("ocr-serving-amount").value=100;by("ocr-serving-unit").value=parsed.servingUnit||"g";}alpha0631ScaleOcrReview();by("ocr-review").classList.remove("hidden");
}
by("ocr-basis")?.addEventListener("change",()=>ocrParsedPanel&&fillOcrReview(ocrParsedPanel,by("ocr-basis").value));
by("ocr-serving-amount")?.addEventListener("input",alpha0631ScaleOcrReview);
by("ocr-serving-unit")?.addEventListener("change",alpha0631ScaleOcrReview);
...
const HELP={
  ...
  "quick-log":"First confirm the date and meal. Then choose Keyboard Entry, Voice Entry, Barcode Scanning or Nutrition Panel. All four methods feed into the same HEC food intelligence and review process; nothing is added until you confirm it.",
...
function alpha0615OpenMealOverview(meal){
  ...
  openModal(`${meal} · ${formatNumber(total)} Cal`,`${relativeDateLabel(date)} · Tap an item to edit it. Swipe left on a row to reveal Delete.`,`Done`,()=>{},`<div class="meal-overview-list">${items.length?items.map(e=>`<div class="meal-overview-row" data-overview-row="${esc(e.id)}"><button type="button" class="meal-overview-edit" data-overview-edit="${esc(e.id)}"><span><strong>${esc(e.name)}</strong><small>${esc(entryNaturalQuantity(e))}</small></span><b>${energyText(e.nutrients?.calories)}</b></button><button type="button" class="meal-overview-delete" data-overview-delete="${esc(e.id)}">🗑 Delete</button></div>`).join(''):`<p class="empty-state">No Entries Yet.</p>`}</div><button type="button" class="secondary wide" data-overview-add="${esc(meal)}">＋ Add Food To ${esc(meal)}</button>`);
...
function alpha0627ShowQuickMethod(method){
  const card=by('quick-log-entry'),title=by('quick-log-entry-title'),voice=by('start-voice-log'),stop=by('stop-voice-log'),status=by('voice-status');if(!card)return;
  ext.ui.quickLogMethod=method;saveExt();card.classList.remove('hidden');by('voice-review')?.classList.add('hidden');
  if(method==='voice'){if(title)title.textContent='Voice Entry';voice?.classList.remove('hidden');if(status)status.textContent='Speak naturally. HEC will send what it hears through the same food search engine as keyboard entry.';setTimeout(()=>startVoice(),80);}
  else{if(title)title.textContent='Keyboard Entry';voice?.classList.add('hidden');stop?.classList.add('hidden');if(status)status.textContent='Type the food naturally. HEC will open the same predictive Food Search used everywhere else.';by('voice-transcript')?.focus();}
}
initialiseVoice=function(){
  alpha0615InitialiseVoice();const fromDiary=ext.ui.quickLogOrigin==='food-diary',date=fromDiary?(ext.ui.diaryDate||isoToday()):isoToday(),meal=fromDiary?(ext.ui.pendingMeal||''):'';if(by('voice-date'))by('voice-date').value=date;if(by('voice-meal'))by('voice-meal').value=meal;
  by('quick-log-context-card')?.classList.add('hidden');by('quick-log-entry')?.classList.add('hidden');by('voice-review')?.classList.add('hidden');by('start-voice-log')?.classList.add('hidden');by('stop-voice-log')?.classList.add('hidden');if(by('voice-transcript'))by('voice-transcript').value='';
  const d=by('quick-log-review-destination');if(d)d.textContent=(by('voice-meal')?.value?`${relativeDateLabel(date)} · ${by('voice-meal').value}`:'Choose the meal above before logging.');
};
...
by('voice-date')?.addEventListener('change',()=>{const d=alpha0627QuickDestination(false),x=by('quick-log-review-destination');if(x&&d)x.textContent=d.meal?`${relativeDateLabel(d.date)} · ${d.meal}`:'Choose the meal above before logging.';});
by('voice-meal')?.addEventListener('change',()=>{const d=alpha0627QuickDestination(false),x=by('quick-log-review-destination');if(x&&d)x.textContent=d.meal?`${relativeDateLabel(d.date)} · ${d.meal}`:'Choose the meal above before logging.';if(d?.meal&&by('quick-log-method-status')&&['barcode','label'].includes(ext.ui.quickLogMethod))by('quick-log-method-status').textContent=`Destination ready: ${relativeDateLabel(d.date)} · ${d.meal}. Tap ${ext.ui.quickLogMethod==='barcode'?'Barcode Scanning':'Nutrition Panel Reading'} again to continue.`;});
document.addEventListener('click',event=>{
  const method=event.target.closest('[data-quick-log-method]');if(method){event.preventDefault();const m=method.dataset.quickLogMethod;ext.ui.quickLogMethod=m;saveExt();const dest=alpha0627QuickDestination(false);if(!dest?.meal){by('quick-log-context-card')?.classList.remove('hidden');if(by('quick-log-method-status'))by('quick-log-method-status').textContent=`${m==='voice'?'Voice':m==='barcode'?'Barcode':m==='label'?'Nutrition panel':'Keyboard'} selected. Now confirm the date and meal.`;by('voice-meal')?.focus();if(m==='keyboard'||m==='voice')alpha0627ShowQuickMethod(m);return;}ext.ui.diaryDate=dest.date;ext.ui.pendingMeal=dest.meal;ext.ui.mealEntrySession={date:dest.date,meal:dest.meal,source:'quick-log'};saveExt();if(m==='barcode'||m==='label'){ext.ui.scanMode=m==='barcode'?'barcode':'label';ext.ui.scanReturn='quick-log';saveExt();openFeature('scan-centre');if(m==='barcode')setTimeout(()=>startBarcodeCamera(),100);return;}alpha0627ShowQuickMethod(m);return;}
  if(event.target.closest('#quick-log-change-method')){try{recognition?.stop();}catch{}by('quick-log-entry')?.classList.add('hidden');by('voice-review')?.classList.add('hidden');return;}
},true);
// Keyboard and voice are simply two input methods for the canonical Food Search.
// Capture the old Review button before legacy voice parsing can create a second brain.
document.addEventListener('click',event=>{const review=event.target.closest('#parse-voice-log');if(!review)return;event.preventDefault();event.stopImmediatePropagation();const dest=alpha0627QuickDestination(true);if(!dest)return;let query=String(by('voice-transcript')?.value||'').trim();if(!query){showActionToast('Type or say the food first.',null,3500);return;}const compound=S23?.splitCompoundQuery?.(query)||[];if(compound.length>1){query=compound[0];ext.ui.compoundFoodQueue=compound.slice(1);}else ext.ui.compoundFoodQueue=[];try{recognition?.stop();}catch{}ext.ui.diaryDate=dest.date;ext.ui.pendingMeal=dest.meal;ext.ui.mealEntrySession={date:dest.date,meal:dest.meal,source:'quick-log'};ext.ui.libraryTab='all';ext.ui.foodSearch=query;saveExt();openFeature('food-library');setTimeout(()=>{if(by('food-search')){by('food-search').value=query;by('food-search').focus();}renderLibrary();renderFoodLiveMatches(query);},60);},true);
by('quick-log-cancel')?.addEventListener('click',()=>{try{recognition?.stop();}catch{}voiceParsed=[];by('voice-review')?.classList.add('hidden');by('quick-log-entry')?.classList.add('hidden');if(by('voice-transcript'))by('voice-transcript').value='';ext.ui.diaryDate=by('voice-date')?.value||ext.ui.diaryDate||isoToday();const dest=ext.ui.quickLogOrigin||'home';saveExt();openFeature(dest);});
...
function alpha0618RecentTargetMeal(entry){const filter=ext.ui.recentMealFilter||'';if(filter&&filter!=='All')return filter;return ext.ui.pendingMeal||entry?.meal||'Snacks';}
...
const ALPHA0618_CHAIN_MENUS={
  ...
  mcd:{label:"McDonald’s Australia",aliases:['mcdonalds','mcdonald s','maccas','macca s'],updated:'21 Aug 2026',groups:{
    'What’s New':['El Maco® Beef','El Maco® McCrispy®','El Maco® Chicken McWrap®','Big Arch®','Chicken Big Mac®'],
    'Beef Burgers':['Big Mac®','Double Big Mac®','Quarter Pounder®','Double Quarter Pounder®','Quarter Pounder® Deluxe','Double Quarter Pounder® Deluxe','Quarter Pounder® BBQ Bacon','Double Quarter Pounder® BBQ Bacon','Big Arch®','Cheeseburger','Double Cheeseburger','Triple Cheeseburger','Hamburger','Classic Angus','BBQ Bacon Angus','Big Brekkie Burger'],
    'Chicken & Fish':['El Maco® McCrispy®','El Maco® Chicken McWrap®','Chicken Big Mac®','McCrispy®','McCrispy® Deluxe','McSpicy®','McChicken®','Double McChicken®',"Chicken 'n' Cheese",'Filet-O-Fish®','Double Filet-O-Fish®','Chicken McWings®','3pc Chicken McNuggets®','6pc Chicken McNuggets®','10pc Chicken McNuggets®','20pc Chicken McNuggets®','40pc Chicken McNuggets®','Spicy Chicken McWrap®'],
    'Breakfast':['Bacon & Egg McMuffin®','Sausage McMuffin®','Sausage & Egg McMuffin®','Mighty McMuffin®','Big Brekkie Burger','Brekkie McWrap®','Hotcakes with Butter & Syrup','Hash Brown'],
    'Sides':['Fries','Hash Brown','Garden Salad'],
    'Desserts':['Hot Apple Pie','Sundae','McFlurry®','Soft Serve Cone'],
    'Drinks':['Coca-Cola®','Coca-Cola® No Sugar','Sprite®','Fanta®','Kirks® Lemon, Lime & Bitters','Frozen Fanta® Lemon','Orange Juice','Apple Juice','Bottled Water'],
    'McCafé':['Cappuccino','Latte','Flat White','Long Black','Espresso','Mocha','Hot Chocolate','Iced Latte','Iced Long Black','Strawberry Matcha Iced Latte','Matcha Iced Latte','Matcha Latte'],
    'Condiments':['Ranch Sauce','Spicy Buffalo Sauce','Big Mac® Special Sauce','McChicken® Sauce',"Sweet 'n' Sour Sauce",'Barbeque Sauce','Ketchup','Sweet Mustard Sauce','Tartare Sauce','Aioli Sauce','Balsamic Dressing','Creamy Caesar Dressing'],
    'Bundles':['McSmart® Meal',"Macca's® Mega Meal",'Macca’s® Bundle for 2','Macca’s® Bundle for 4','Macca’s® Bundle for 6']
  }},
...
function s23Parsed(v){return S23?.parseQuery?S23.parseQuery(v):{raw:String(v||''),normalised:s23Norm(v),food:s23Singular(v),quantity:1,unit:'',tokens:s23Singular(v).split(' ').filter(Boolean)};}
...
function alpha0629EntityMatch(entity,food){return REG29?.entityMatchesHay?REG29.entityMatchesHay(entity,`${food?.brand||''} ${food?.name||''} ${(food?.aliases||[]).join(' ')}`):false;}
searchRank=function(food,query){
  const ctx=alpha0630QueryContext(query),p=ctx.parsed,nq=p.food;if(!nq)return 100;
  const name=s23Singular(food?.name||''),brand=s23Singular(food?.brand||''),aliases=(food?.aliases||[]).map(s23Singular),combined=s23Singular(`${food?.brand||''} ${food?.name||''}`);
  const fields=[name,brand,combined,...aliases].filter(Boolean),fieldTokens=[...new Set(fields.flatMap(x=>x.split(' ').filter(Boolean)))];
  ...
  let base=0;
  if(!qt.length)base=identityEntities.length?900:0;
  else if(name===nq||combined===nq)base=1300;
  else if(aliases.includes(nq))base=1250;
  else if(name.startsWith(nq+' ')||combined.startsWith(nq+' '))base=1050;
  else if(qt.every(t=>fieldTokens.includes(t)))base=900+qt.length*10;
  else if(qt.every(t=>fieldTokens.some(f=>f.startsWith(t)&&t.length>=3)))base=790+qt.length*8;
  else if(qt.every(t=>t.length>=4&&fieldTokens.some(f=>fuzzyTokenMatch(t,f))))base=680+qt.length*5;
...
function s23BrandIndex(){
  ...
  for(const e of REG29?.entries||[])if(['brand','retailer'].includes(e.type))vals.push(e.name,...(e.aliases||[]));
...
function s23BrandMatch(raw){
  const registered=REG29?.primary?REG29.primary(raw,['brand','retailer']):null;if(registered)return registered.entity.name;
...
function s23LikelyProduct(raw,concept=null){
  ...
  let answer=false;
  if(alpha0618ChainFor(raw)||REG29?.primary?.(raw,['brand','retailer'])||s23BrandMatch(raw))answer=true;
  else{const c=concept||s23Concept(raw),strongGeneric=allFoods().filter(f=>f.category!=='Recipe'&&!s23ProductLike(f)).some(f=>searchRank(f,p.food)>=900);if(!strongGeneric){const prefix=S23?.likelyBrandPrefix?.(p,c)||'';answer=!!prefix||(!c&&!s23DynamicConcept(raw));}}
...
function s23ProductMatches(raw,limit=16){
  const ctx=alpha0630QueryContext(raw),q=ctx.parsed.food,residual=ctx.residualTokens,entities=ctx.entities.filter(x=>['brand','retailer'].includes(x.entity.type));if(!q)return[];
  const cacheKey=`${alpha0630FoodSearchRevision}|${s23Norm(raw)}`;let ranked=alpha0630ProductMatchCache.get(cacheKey);
  if(!ranked){ranked=allFoods().filter(s23ProductLike).map(food=>{
    const hay=s23Norm(`${food.brand||''} ${food.name||''} ${(food.aliases||[]).join(' ')}`),hayTokens=hay.split(' ');
  ...
  }).filter(x=>x.score>0&&x.entityMatched&&x.missing===0).sort((a,b)=>b.score-a.score||Number(b.food.country==='Australia')-Number(a.food.country==='Australia')||a.food.name.localeCompare(b.food.name)).map(x=>x.food).filter((f,i,a)=>a.findIndex(x=>s23Norm(`${x.brand}|${x.name}`)===s23Norm(`${f.brand}|${f.name}`))===i);
...
function s23EnergyMeta(food){return [food.brand,cleanMeasureText(food.serving),energyText(food.nutrients?.calories)].filter(Boolean).join(' · ');}
function s23ProductRow(food){return `<button type="button" class="live-match-row" data-food-add="${esc(food.id)}"><span><strong>${esc(food.name)}</strong><small>${esc(s23EnergyMeta(food))}</small></span><b>＋</b></button>`;}
...
function alpha0627SourceRows(raw,concept){
  ...
  const choices=S23.sourceChoices?.(concept)||[];const icons={'Homemade':'🏠','Home Grown':'🌱','Commercial / Bought':'🏷️','Commercial / Packaged':'🏷️','Takeaway / Restaurant':'🍽️','Not Sure / Typical':'↔'};
  ...
  return `<div class="alpha0627-source-first"><small>What best describes this food?</small>${choices.map(v=>`<button type="button" class="live-match-row" data-alpha0627-source="${esc(v)}" data-alpha0627-query="${esc(raw)}"><span><strong>${icons[v]||'•'} ${esc(v)}</strong><small>${esc(help(v))}</small></span><b>›</b></button>`).join('')}</div>`;
}
...
function s23Naturalise(source,w){
  ...
  if(natural.grams===1&&natural.unit==='g'){units.g=.01;labels.g='g';f.defaultUnit='g';f.defaultAmount=100;f.serving='Enter amount in grams';}
...
function s23FinishWizard(w){
  ...
  prepareEntry(guided,{date,meal:ext.ui.pendingMeal||'',amount:w.parsed.quantity||null,unit:requestedUnit||defaultUnit(guided)});
}
...
function s23RenderLive(raw){
  ...
  if(/\bburger king\b/.test(q)){box.innerHTML='<div class="live-match-heading"><strong>Top Match</strong><small>Australian search · Alpha 0.6.32</small></div><button type="button" class="live-match-row" data-alpha0623-chain-redirect="Hungry Jacks"><span><strong>Hungry Jack’s</strong><small>Australian equivalent</small></span><b>›</b></button>';box.classList.remove('hidden');return;}
  ...
  // A recognised menu item can find its Australian chain without requiring the
  // user to type the chain first: McMuffin → McDonald’s, Whopper → Hungry Jack’s,
  // Zinger → KFC. An exact generic food concept still wins (e.g. Sausage must
  ...
  if(concept&&!product){const source=alpha0627SourceRows(term,concept);box.innerHTML=`<div class="live-match-heading"><strong>${esc(s23GuideLabel(term,concept))}</strong><small>HEC Food Intelligence · Alpha 0.6.32</small></div>${source||s23GuideButton(term,true)}`;box.classList.remove('hidden');return;}
...
ext.version='0.6.32';ext.ui.universalGuidedSearch=true;ext.ui.guidedSearchArchitecture='food-intelligence-v629';ext.ui.guidedMatchValidation='strict-compatible-source-v629';saveExt();
...
makeOpenFoodFactsFood=function(product){
  ...
  const nutrients={calories:Number(calories)||0,protein:pick('proteins'),carbs:pick('carbohydrates'),fat:pick('fat'),satFat:pick('saturated-fat'),fibre:pick('fiber'),sugar:pick('sugars'),sodium:sodium===null?null:Number(sodium)*1000};
  const countryText=[product.countries,...(product.countries_tags||[])].filter(Boolean).join(' ');
...
ext.ui.entityRegistryVersion=REG29?.version||'0.6.32';
...
entryCard=function(entry){const label=alpha0631DisplayIdentity(entry);return `<article class="simple-diary-entry recorded-entry" data-entry-id="${esc(entry.id)}"><button class="entry-open" data-entry-edit="${esc(entry.id)}"><span><strong>${esc(label)}</strong><small>${formatNaturalAmount(entry.amount)} ${esc(friendlyUnitLabel(getFood(entry.foodId)||{},entry.unit,entry.amount))}</small></span><b>${energyText(entry.nutrients?.calories)}</b></button><button class="entry-more" data-entry-menu="${esc(entry.id)}" aria-label="More actions for ${esc(label)}">•••</button><div class="entry-inline-actions hidden" data-entry-actions="${esc(entry.id)}"><button data-entry-copy="${esc(entry.id)}">Copy</button><button data-entry-move="${esc(entry.id)}">Move</button><button data-entry-delete="${esc(entry.id)}" class="delete-action">Delete Food</button></div></article>`;};
alpha0615OpenMealOverview=function(meal){const date=ext.ui.diaryDate||isoToday(),items=entriesForDate(date).filter(e=>e.meal===meal&&e.status!=='skipped'),total=sumNutrients(items).calories;openModal(`${meal} · ${formatNumber(total)} Cal`,`${relativeDateLabel(date)} · Tap an item to edit it. Swipe left on a row to reveal Delete.`,`Done`,()=>{},`<div class="meal-overview-list">${items.length?items.map(e=>`<div class="meal-overview-row" data-overview-row="${esc(e.id)}"><button type="button" class="meal-overview-edit" data-overview-edit="${esc(e.id)}"><span><strong>${esc(alpha0631DisplayIdentity(e))}</strong><small>${esc(entryNaturalQuantity(e))}</small></span><b>${energyText(e.nutrients?.calories)}</b></button><button type="button" class="meal-overview-delete" data-overview-delete="${esc(e.id)}">🗑 Delete</button></div>`).join(''):`<p class="empty-state">No Entries Yet.</p>`}</div><button type="button" class="secondary wide" data-overview-add="${esc(meal)}">＋ Add Food To ${esc(meal)}</button>${items.length?`<div class="quick-action-row alpha0632-overview-transfer"><button type="button" class="secondary" data-copy-diary-meal="${esc(meal)}">Copy Meal</button><button type="button" class="secondary" data-move-diary-meal="${esc(meal)}">Move Meal</button></div>`:''}`);setTimeout(()=>alpha0615EnableSwipeDelete(),0);};

const alpha0631PrepareEntryBase=prepareEntry;
prepareEntry=function(food,opts={}){alpha0631PrepareEntryBase(food,opts);const unit=by('entry-unit'),label=unit?.closest('label');if(!label)return;const locked=food?.lockedServingUnit&&unitOptions(food)?.[food.lockedServingUnit]!==undefined;if(locked){unit.value=food.lockedServingUnit;label.classList.add('alpha0631-locked-unit');label.setAttribute('aria-hidden','true');}else{label.classList.remove('alpha0631-locked-unit');label.removeAttribute('aria-hidden');}updateEntryPreview();};
...
window.HEC_ALPHA0631_TEST={build:'0.6.32',externalQuery:alpha0631ExternalSearchQuery,conceptCandidates:q=>{const c=s23Concept(q);return c?s23ConceptCandidates(q,c).map(f=>f.name):[];},displayIdentity:alpha0631DisplayIdentity,frontName:alpha0631FrontNameFromText};
ext.version='0.6.32';ext.ui.searchPolish='incremental-online-v631';ext.ui.nutritionPanelScaling='basis-aware-v631';ext.ui.productFrontAssist='optional-front-ocr-v631';saveExt();
