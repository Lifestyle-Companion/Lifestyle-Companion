...
const APP = window.HEC_APP || {name:"Healthy Eating Companion",version:"0.6.7",storageKey:"healthyEatingCompanionAlpha06",functionalStorageKey:"healthyEatingCompanionAlpha06Functional"};
...
// Alpha 0.6.7: keep a plain egg separate from egg dishes so search and preparation are easier to understand.
FOODS.push(
...
const EXT_DEFAULTS = {
  version:"0.6.7", diary:{}, daySettings:{}, water:{}, fluidTargets:{}, steps:{}, dailyNotes:{}, exercise:[], shopping:[], onlineFoods:[], onlineSearchCache:{},
...
function loadExt(){
  ...
  if(current){ const loaded=merge(clone(EXT_DEFAULTS),current); loaded.version="0.6.7"; return loaded; }
  for(const legacyKey of LEGACY_EXT_KEYS){
    try {
      const legacy = JSON.parse(localStorage.getItem(legacyKey));
      if(legacy){
        const migrated = merge(clone(EXT_DEFAULTS),legacy);
        migrated.version = "0.6.7";
...
function renderDiary(){
  ...
  by("diary-meals").innerHTML=mealNames().map(meal=>{const allMealEntries=entries.filter(e=>e.meal===meal),shown=allMealEntries,projectedTotals=sumNutrients(allMealEntries.filter(e=>e.status!=="skipped")),eaten=allMealEntries.filter(e=>e.status==="eaten").length,planned=allMealEntries.filter(e=>e.status==="planned").length,skipped=allMealEntries.filter(e=>e.status==="skipped").length;let state="Nothing Planned";if(planned&&eaten)state=`${eaten} Eaten · ${planned} Planned`;else if(planned)state=`${planned} Planned`;else if(eaten)state="Complete ✓";else if(skipped)state="Skipped";return `<section class="meal-list-section redesigned-meal-section" data-meal-name="${esc(meal)}"><header class="meal-list-heading redesigned-meal-heading"><div><h3>${esc(meal)}</h3><strong>${formatNumber(projectedTotals.calories)} Cal</strong></div><small>${esc(state)}</small></header><div class="meal-simple-list">${shown.length?shown.map(entryCard).join(""):`<p class="meal-empty">Nothing Planned Yet.</p>`}</div><footer class="meal-list-actions redesigned-meal-actions"><button class="meal-add-text" data-add-to-meal="${esc(meal)}" aria-label="Add food to ${esc(meal)}">＋ Add Food</button><button class="meal-suggest-text" data-suggest-context-meal="${esc(meal)}" aria-label="Ask the companion to suggest ${esc(meal)}">✨ Suggest</button>${planned?`<button class="meal-confirm-text" data-mark-meal-eaten="${esc(meal)}">✓ Ate as Planned</button>`:""}<button class="meal-more-text" data-meal-menu="${esc(meal)}" aria-label="More ${esc(meal)} actions">•••</button></footer><div class="meal-menu-actions hidden" data-meal-actions="${esc(meal)}">${allMealEntries.length?`<button data-save-meal-template="${esc(meal)}">Save as Reusable Meal</button>`:`<span>No Additional Meal Actions Yet.</span>`}</div></section>`;}).join("");
...
by("photo-add-note")?.addEventListener("click",()=>showActionToast("Meal photos are kept only during this browser session in Alpha 0.6.7. Add a written Diary note for lasting context.",null,8000));
...
// Contextual help
const HELP={
  "food-diary":"The Diary is the main place to build your day. Add food directly inside a meal, or tap Suggest to ask the companion for that meal. Future and planning entries are Planned automatically; use Ate as Planned when the meal happened as expected. Edit Day opens Normal/Fasting settings only when you need them. Plan Multiple Meals opens the separate bulk planner.",
...
// Alpha 0.6.7 migration: keep existing records, repair the stale Normal-Day target, and preserve explicit per-date overrides only when marked custom.
