...
const APP = window.HEC_APP || {name:"Healthy Eating Companion",version:"0.6.9",storageKey:"healthyEatingCompanionAlpha06",functionalStorageKey:"healthyEatingCompanionAlpha06Functional"};
...
// Alpha 0.6.9: keep a plain egg separate from egg dishes so search and preparation are easier to understand.
FOODS.push(
...
const EXT_DEFAULTS = {
  version:"0.6.9", diary:{}, daySettings:{}, water:{}, fluidTargets:{}, steps:{}, dailyNotes:{}, exercise:[], shopping:[], onlineFoods:[], onlineSearchCache:{},
...
function loadExt(){
  ...
  if(current){ const loaded=merge(clone(EXT_DEFAULTS),current); loaded.version="0.6.9"; return loaded; }
  for(const legacyKey of LEGACY_EXT_KEYS){
    try {
      const legacy = JSON.parse(localStorage.getItem(legacyKey));
      if(legacy){
        const migrated = merge(clone(EXT_DEFAULTS),legacy);
        migrated.version = "0.6.9";
...
document.addEventListener("click",event => {
  ...
  if(scanButton){event.preventDefault();ext.ui.scanMode=scanButton.dataset.openScanMode||"barcode";saveExt();openFeature("scan-centre");return;}
...
function renderLibrary(){
  ...
  if(!query&&tab==="all"){
    ...
    intro=`<div class="search-guidance everyday-food-guidance"><strong>Everyday Australian Foods</strong><small>Browse familiar starter foods below, or search all ${AFCD_FOODS.length?AFCD_FOODS.length.toLocaleString():"1,588"} Australian AFCD records by name.</small></div>`;
  }
  let closeNote="";
...
function keepLiveFoodResultsVisible(){
  requestAnimationFrame(()=>{const input=by("food-search"),results=by("food-results");if(!input||document.activeElement!==input||!results)return;const first=results.querySelector(".search-guidance,.resource-row,.resource-empty");if(first){const vv=window.visualViewport;const keyboardTop=vv?vv.offsetTop+vv.height:window.innerHeight;const rect=first.getBoundingClientRect();if(rect.bottom>keyboardTop-18||rect.top<0)first.scrollIntoView({block:"nearest",behavior:"smooth"});}});
}
...
function mealFromText(text){const t=normalise(text);if(t.includes("breakfast"))return"Breakfast";if(t.includes("smoko"))return t.includes("afternoon")?"Afternoon Tea":"Morning Tea";if(t.includes("lunch"))return"Lunch";if(t.includes("dinner")||t.includes("tea"))return"Dinner";if(t.includes("supper"))return"Snacks";return "";}
...
let scanFile=null,scanBarcodeControls=null,scanBarcodeFood=null,ocrParsedPanel=null;
...
function updateScanModeUI(){
  const mode=ext.ui.scanMode||"food";qa("[data-scan-mode]").forEach(b=>b.classList.toggle("active",b.dataset.scanMode===mode));by("barcode-tools")?.classList.toggle("hidden",mode!=="barcode");by("label-tools")?.classList.toggle("hidden",mode!=="label");by("photo-tools")?.classList.toggle("hidden",mode!=="food");
  const copy={food:"Take a meal photo, then identify and confirm every food before anything is logged.",barcode:"Point the camera at a retail barcode, or photograph/enter it. A matching packaged product is looked up online and must be checked against the package.",label:"Photograph the Nutrition Information Panel square-on in good light. OCR reads it on this device, then you review every value before saving."}[mode];if(by("scan-mode-copy"))by("scan-mode-copy").textContent=copy;
  if(mode==="barcode")by("barcode-status").textContent="Ready to scan. If the barcode is not in the online product database, use Nutrition Panel instead.";
}
...
async function lookupBarcodeProduct(code){
  const clean=String(code||"").replace(/\D/g,"");if(clean.length<8||clean.length>14){showActionToast("Enter Or Scan A Valid Barcode.",null,5000);return null;}by("barcode-status").textContent=`Looking Up ${clean}…`;by("scan-review-card")?.classList.add("hidden");scanBarcodeFood=null;
  ...
  try{
    ...
    by("scan-review-card").classList.remove("hidden");by("barcode-status").textContent=`Found ${food.name}. Check It Against The Package Before Continuing.`;return food;
...
by("start-barcode-camera")?.addEventListener("click",async()=>{
  const video=by("barcode-video");video.classList.remove("hidden");by("stop-barcode-camera").classList.remove("hidden");by("barcode-status").textContent="Opening Camera… Hold The Barcode Steady Inside The View.";
  try{const ok=await ensureBarcodeLibrary();if(!ok)throw new Error("Scanner unavailable");const reader=new ZXingBrowser.BrowserMultiFormatReader();scanBarcodeControls=await reader.decodeFromVideoDevice(undefined,video,(result)=>{const text=result?.getText?.()||result?.text;if(text){by("scan-barcode-input").value=text;scanBarcodeControls?.stop?.();scanBarcodeControls=null;video.classList.add("hidden");by("stop-barcode-camera").classList.add("hidden");lookupBarcodeProduct(text);}});by("barcode-status").textContent="Camera Ready. Hold A Barcode Steady Until It Is Detected.";}catch{video.classList.add("hidden");by("stop-barcode-camera").classList.add("hidden");by("barcode-status").textContent="Live Camera Scanning Is Unavailable. Check Camera Permission, Take A Barcode Photo, Or Enter The Number.";}
});
by("stop-barcode-camera")?.addEventListener("click",()=>{scanBarcodeControls?.stop?.();scanBarcodeControls=null;by("barcode-video")?.classList.add("hidden");by("stop-barcode-camera")?.classList.add("hidden");by("barcode-status").textContent="Barcode Camera Stopped.";});
...
by("photo-add-note")?.addEventListener("click",()=>showActionToast("Meal Photos Stay In This Browser Session Only In Alpha 0.6.9.",null,5000));
...
function renderDailyProgress(){
  ...
  const allEntries=entriesForDate(date).filter(e=>e.status!=="skipped"),meals=plannerMealNames().concat(allEntries.some(e=>e.meal==="Other")?["Other"]:[]);
...
function renderHistory(period){
  ...
  if(weights.length){const values=weights.map(x=>n(x.weightKg)),rawMin=Math.min(...values),rawMax=Math.max(...values),pad=Math.max(.5,(rawMax-rawMin)*.35),min=rawMin-pad,max=rawMax+pad,span=Math.max(.5,max-min),w=Math.max(320,Math.min(760,weights.length*86+80)),h=240,left=46,right=18,top=22,bottom=58,plotW=w-left-right,plotH=h-top-bottom,points=weights.map((x,i)=>{const px=left+(weights.length===1?plotW/2:i*plotW/(weights.length-1)),py=top+(max-n(x.weightKg))/span*plotH;return {...x,px,py};}),poly=points.map(p=>`${p.px.toFixed(1)},${p.py.toFixed(1)}`).join(" "),ticks=[max,(max+min)/2,min];by("history-bars").innerHTML=`<div class="weight-chart-scroll"><svg class="weight-trend-svg" viewBox="0 0 ${w} ${h}" role="img" aria-label="Weight trend from ${formatDate(weights[0].date)} to ${formatDate(weights.at(-1).date)}">${ticks.map(v=>{const y=top+(max-v)/span*plotH;return `<line class="weight-grid-line" x1="${left}" x2="${w-right}" y1="${y}" y2="${y}"></line><text class="weight-axis-label" x="${left-8}" y="${y+4}" text-anchor="end">${Number(v).toFixed(1)}</text>`;}).join("")}<polyline class="weight-trend-line" points="${poly}"></polyline>${points.map(p=>`<circle class="weight-trend-point" cx="${p.px}" cy="${p.py}" r="5"></circle><text class="weight-point-label" x="${p.px}" y="${p.py-11}" text-anchor="middle">${Number(p.weightKg).toFixed(1)} kg</text><text class="weight-date-label" x="${p.px}" y="${h-20}" text-anchor="middle">${esc(new Intl.DateTimeFormat("en-AU",{weekday:"short",day:"numeric",month:"short"}).format(new Date(p.date+"T12:00:00")).replace(",",""))}</text>`).join("")}</svg></div>`;}else by("history-bars").innerHTML=`<p class="empty-state">Add Weight Check-Ins to build your weight trend.</p>`;
...
// Alpha 0.6.9 migration: keep existing records, simplify all active Diary entries to one recorded state, and preserve explicit day targets.
