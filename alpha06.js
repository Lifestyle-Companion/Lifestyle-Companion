...
const APP = window.HEC_APP || {name:"Healthy Eating Companion",version:"0.6.6",storageKey:"healthyEatingCompanionAlpha06",functionalStorageKey:"healthyEatingCompanionAlpha06Functional"};
...
// Alpha 0.6.6: keep a plain egg separate from egg dishes so search and preparation are easier to understand.
FOODS.push(
...
const EXT_DEFAULTS = {
  version:"0.6.6", diary:{}, daySettings:{}, water:{}, fluidTargets:{}, steps:{}, dailyNotes:{}, exercise:[], shopping:[], onlineFoods:[], onlineSearchCache:{},
  family:{enabled:false,name:"",email:""}, connections:{}, customFoods:[], savedFoodIds:[], recipes:[], mealTemplates:[], shoppingVoiceAliases:{},
  ui:{diaryDate:isoToday(),progressDate:isoToday(),plannerDate:isoToday(),diaryView:"all",libraryTab:"all",scanMode:"food",pendingMeal:"",plannerResults:{},plannerRejected:{},plannerAccepted:{},plannerSessionActive:false,recipeDraft:[],recipeName:"",recipeServings:4,recipeNotes:"",returnToRecipe:false,replacingEntryId:""}
};
...
function loadExt(){
  ...
  if(current){ const loaded=merge(clone(EXT_DEFAULTS),current); loaded.version="0.6.6"; return loaded; }
  for(const legacyKey of LEGACY_EXT_KEYS){
    try {
      const legacy = JSON.parse(localStorage.getItem(legacyKey));
      if(legacy){
        const migrated = merge(clone(EXT_DEFAULTS),legacy);
        migrated.version = "0.6.6";
...
function currentGoals(date=isoToday()){
  const settings=ext.daySettings[date]||{},type=settings.type||"normal",baseDefault=type==="fasting"?(n(ext.dayTypeTargets?.fasting)||500):recommendedNormalTarget(),calTarget=settings.customTarget?n(settings.targetCal)||baseDefault:baseDefault;
  const exerciseCredit=(ext.exercise||[]).filter(x=>x.date?.slice(0,10)===date).reduce((sum,x)=>sum+n(x.credit),0),hydration=hydrationReference(),macros=balancedMacroTargets(calTarget);
  return {calories:calTarget+exerciseCredit,baseCalories:calTarget,exerciseCredit,hydration:hydration.fluids,fluids:hydration.fluids,protein:macros.protein,fat:macros.fat,carbs:macros.carbs,fibre:30,sugar:null,freeSugar:calTarget*.10/4,addedSugar:calTarget*.10/4,sodium:2000,steps:10000,foodGroups:foodGroupGoals(),dayType:type};
}
...
function renderDiary(){
  ...
  const savedSettings=ext.daySettings[date]||{},type=savedSettings.type||"normal",baseTarget=type==="fasting"?(n(ext.dayTypeTargets?.fasting)||500):recommendedNormalTarget(),customTarget=!!savedSettings.customTarget,activeTarget=customTarget?(n(savedSettings.targetCal)||baseTarget):baseTarget;
  by("day-type").value=type;by("day-custom-target").checked=customTarget;by("day-cal-target").value=whole(activeTarget);by("day-cal-target-label").classList.toggle("hidden",!customTarget);by("day-base-target").textContent=`${formatNumber(baseTarget)} Cal`;by("day-target-label").textContent=type==="fasting"?"Preferred Fasting-Day Target":"Recommended Normal Target";
  daySettingsBaseline={type,targetCal:whole(activeTarget),customTarget};setDaySettingsDirty(false);
  by("day-settings-note").textContent=type==="fasting"?"Fasting Day. Unticked meals in the Meal Planner are treated as meals you intend to skip unless you select them.":"Normal Day. The profile recommendation is used unless you deliberately choose a different target for this date.";
...
by("day-type")?.addEventListener("change",()=>{
  const type=by("day-type").value,target=type==="fasting"?(n(ext.dayTypeTargets?.fasting)||500):recommendedNormalTarget();by("day-custom-target").checked=false;by("day-cal-target-label").classList.add("hidden");by("day-cal-target").value=whole(target);by("day-base-target").textContent=`${formatNumber(target)} Cal`;by("day-target-label").textContent=type==="fasting"?"Preferred Fasting-Day Target":"Recommended Normal Target";by("day-settings-note").textContent=type==="fasting"?"Fasting Day. Unticked meals in the Meal Planner are treated as meals you intend to skip unless you select them.":"Normal Day. The profile recommendation is used unless you deliberately choose a different target for this date.";updateDaySettingsDirty();refreshDiaryEnergyPreview(target);
});
by("day-custom-target")?.addEventListener("change",()=>{const custom=by("day-custom-target").checked;by("day-cal-target-label").classList.toggle("hidden",!custom);if(!custom){const type=by("day-type").value,target=type==="fasting"?(n(ext.dayTypeTargets?.fasting)||500):recommendedNormalTarget();by("day-cal-target").value=whole(target);refreshDiaryEnergyPreview(target);}updateDaySettingsDirty();});
...
document.addEventListener("click",event => {
  ...
  if(add){ ext.ui.recentPlanMode=false;ext.ui.pendingMeal=add.dataset.addToMeal;ext.ui.libraryTab="all";ext.ui.foodSearch="";saveExt();openFeature("food-library");return; }
...
function saveDaySettings(showMessage=true){
  const date=diaryDate(),type=by("day-type").value,customTarget=!!by("day-custom-target")?.checked,baseTarget=type==="fasting"?(n(ext.dayTypeTargets?.fasting)||500):recommendedNormalTarget(),targetCal=customTarget?whole(by("day-cal-target").value):whole(baseTarget);ext.daySettings[date]={type,targetCal,customTarget};saveExt();daySettingsBaseline=clone(ext.daySettings[date]);setDaySettingsDirty(false);renderDiary();if(showMessage)showActionToast("Day Settings Saved.",null,5000);
}
...
document.addEventListener("click",event=>{const suggest=event.target.closest("[data-suggest-context-meal]");if(!suggest)return;const meal=suggest.dataset.suggestContextMeal,date=ext.ui.diaryDate||isoToday();ext.ui.plannerDate=date;ext.ui.plannerResults={};ext.ui.plannerRejected={};ext.ui.plannerAccepted={};ext.ui.plannerSessionActive=true;qa('input[name="planner-meal"]').forEach(x=>x.checked=x.value===meal);saveExt();openFeature("meal-planner");setTimeout(()=>{qa('input[name="planner-meal"]').forEach(x=>x.checked=x.value===meal);updatePlannerSelectAll();generatePlannerResults(false);},80);});
...
by("photo-add-note")?.addEventListener("click",()=>showActionToast("Meal photos are kept only during this browser session in Alpha 0.6.6. Add a written Diary note for lasting context.",null,8000));
...
// Alpha 0.6.6 migration: keep existing records, repair the stale Normal-Day target, and preserve explicit per-date overrides only when marked custom.
function refreshDiaryEnergyPreview(target){const slide=by("diary-day-summary")?.querySelector(".summary-slide");if(!slide)return;const date=diaryDate(),consumed=dayNutrition(date,["eaten"]).calories,exercise=(ext.exercise||[]).filter(x=>x.date?.slice(0,10)===date).reduce((sum,x)=>sum+n(x.credit),0),goal=n(target)+exercise;slide.innerHTML=`<span>${date===isoToday()?"Today’s Energy":relativeDateLabel(date).split(" · ")[0]+" Energy"}</span><div class="diary-kpi-row"><div><small>Goal</small><strong>${formatNumber(goal)} Cal</strong></div><div><small>Consumed</small><strong>${formatNumber(consumed)} Cal</strong></div><div><small>Remaining</small><strong>${formatNumber(Math.max(0,goal-consumed))} Cal</strong></div></div>`;}
ext.version="0.6.6";ext.dayTypeTargets||={normal:recommendedNormalTarget(),fasting:500};ext.dayTypeTargets.normal=recommendedNormalTarget();if(!n(ext.dayTypeTargets.fasting))ext.dayTypeTargets.fasting=500;Object.values(ext.daySettings||{}).forEach(settings=>{if(!settings)return;if(settings.type==="normal"&&!settings.customTarget)settings.targetCal=recommendedNormalTarget();if(settings.customTarget===undefined)settings.customTarget=false;});normaliseShoppingCategories();saveExt();
