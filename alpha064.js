...
function ensureSchemas(){
  const main=read(MAIN_KEY,{});main.version=APP.version||"0.6.4";main.personal||={};main.personal.surname??="";main.personal.homeTimeZone||=deviceTZ();main.personal.activeTimeZone||=main.personal.homeTimeZone;main.personal.timeZoneBehaviour||="ask";main.analytics||={consent:false};main.developer||={founderEnabled:false};main.trial||={};
  ...
  write(MAIN_KEY,main);
  const ext=read(EXT_KEY,{});ext.version=APP.version||"0.6.4";ext.fluidTargets||={};ext.ui||={};write(EXT_KEY,ext);
...
$("export-all-data")?.addEventListener("click",()=>{downloadJson(`healthy-eating-companion-alpha-0-6-4-backup-${window.HECDate?.todayISO?.()||"today"}.json`,{format:"HEC-BACKUP-1",version:APP.version,exportedAt:new Date().toISOString(),profile:read(MAIN_KEY,{}),functional:read(EXT_KEY,{})});track("data:backup");});
...
$("export-insights")?.addEventListener("click",()=>downloadJson(`hec-alpha-0-6-4-local-insights.json`,read(ADMIN_KEY,{})));
